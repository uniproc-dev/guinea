...
use guinea_core::test_kit::Stabilizer;
use std::ops::Deref;
...
impl UiDispatcher for TestUiDispatcher {
    fn init(&self) {
        ...
        guinea_core::actor::set_ui_dispatcher(self.clone());
    }
...
pub fn init_tracing(settings_path: PathBuf) -> anyhow::Result<()> {
    ...
    let _ = guinea_core::trace::init_test_subscriber(writer, 64, Targets::new(), TEST_CAPTURED_LOGS.clone());
...
pub fn temp_settings_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("guinea-test-kit-{nanos}.json"))
}
