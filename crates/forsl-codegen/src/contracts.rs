...
pub fn emit_capability_registration(key: &str, struct_name: &str) -> TokenStream {
    ...
    quote! {
        ...
        ::inventory::submit! {
            forsl_core::contracts::CapabilityMeta {
                key: #key,
...
pub fn emit_port_registration(item_trait: &ItemTrait) -> TokenStream {
    ...
    let extra_method_entries = extra_methods.iter().map(|m| {
        ...
        quote! {
            forsl_core::contracts::PortExtraMethod {
                name: #name,
    ...
    quote! {
        ...
        ::inventory::submit! {
            forsl_core::contracts::PortStubMeta {
...
pub fn emit_binding_registration(item_trait: &ItemTrait) -> TokenStream {
    ...
    let method_entries = methods.iter().map(|m| {
        ...
        quote! {
            forsl_core::contracts::BindingMethodMeta {
                name: #name,
                arg_types: &[#(#arg_types),*],
                is_manual: #is_manual,
    ...
    quote! {
        ::inventory::submit! {
            forsl_core::contracts::BindingStubMeta {
                trait_name: #trait_name,
                feature: #feature,
