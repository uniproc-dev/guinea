//! Slint-backend codegen engine driven purely by the `guinea_core::contracts`
...
use guinea_core::contracts::{BindingMethodMeta, BindingStubMeta, CapabilityMeta};
...
fn binding_tracing_wrapper(method: &BindingMethodMeta, method_name: &str) -> String {
    ...
    match arity {
        0 => format!(
            r#"        let handler = {{
            let handler = handler;
            move || guinea_core::trace::in_ui_action_scope({scope:?}, {target_expr}, None, || handler())
        }};"#
        ),
        1 => format!(
            r#"        let handler = {{
            ...
            move |__ui_arg0| {{
                ...
                guinea_core::trace::in_ui_action_scope({scope:?}, {target_expr}, __ui_target, || handler(__ui_arg0))
            }}
        }};"#
        ),
        2 => format!(
            ...
            move |__ui_arg0, __ui_arg1| {{
                ...
                guinea_core::trace::in_ui_action_scope({scope:?}, {target_expr}, __ui_target, || handler(__ui_arg0, __ui_arg1))
            }}
        }};"#
        ),
    ...
    format!(
        r#"
            if guinea_core::trace::is_scope_enabled("ui.adapter.call") {{
                ...
                if guinea_core::trace::is_target_enabled(&__t) {{
                    guinea_core::trace::in_named_scope("ui.adapter.call", Some("adapter,method"), Some(__t), || {{
