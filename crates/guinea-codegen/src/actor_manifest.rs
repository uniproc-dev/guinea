        ...
        match it.kind {
            ...
            ManifestItem::New(s) => {
                ...
                generated_structs.push(quote! {
                    ...
                    #(#attrs)* impl guinea_core::actor::Message for #id {}
                });
                ...
                if is_bus {
                    logic_calls.push(quote! {
                        #(#attrs)*
                        <#item_ty as guinea_core::actor::event_bus::builder::EventSubscription<#self_ty>>::subscribe_into(addr.clone(), tracker);
                    });
                } else {
                    logic_calls.push(quote! {
                        #(#attrs)*
...
/// `bindings` is the caller's already-force-linked
/// `guinea_core::contracts::bindings()` iterator - reading the registry
/// (and making sure it isn't empty due to linker dead-stripping) requires a
/// real Cargo dependency on the app's own contracts crate, which this crate
/// intentionally doesn't have; the caller (a thin wrapper proc-macro that
/// *does* depend on it) supplies the already-resolved iterator instead.
pub fn actor_manifest_impl(
    attr: TokenStream,
    mut impl_block: ItemImpl,
    mut bindings: impl Iterator<Item = &'static BindingStubMeta>,
) -> TokenStream {
        ...
        if let ImplItem::Type(ty_item) = item {
            if ty_item.ident == "Bus" {
                ...
                if let Some(res) = transform_manifest(
                    ...
                    bus_logic = quote! {
                        ...
                        impl #impl_generics guinea_core::actor::event_bus::builder::EventSubscription<#self_ty> for #marker_id #where_clause {
                            fn subscribe_into(addr: guinea_core::actor::Addr<#self_ty>, tracker: &impl guinea_core::lifecycle_tracker::LifecycleTracker) {
                                #(#calls)*
                            }
                        }
                    };
            ...
            if ty_item.ident == "Signals" {
                ...
                let sig_impls = msgs.iter().map(|msg_ty| {
                    quote! {
                        impl guinea_core::actor::traits::AllowedSignal<#msg_ty> for #marker_id {}
                    }
                });
            ...
            if ty_item.ident == "Handlers" {
                ...
                if let Some(res) = transform_manifest(
                    ...
                    handlers_logic = quote! {
                        #[doc(hidden)] pub struct #marker_id;
                        impl #impl_generics guinea_core::actor::DirectHandler<#self_ty> for #marker_id #where_clause {}
                        const _: () = {
                            fn check_handlers #impl_generics () #where_clause {
                                fn assert_handler<A, M>() where A: guinea_core::actor::Handler<M>, M: guinea_core::actor::Message {}
                                #(#checks)*
                            }
                        };
                    };
    ...
    if let Some(binder_path) = manifest_args.binder_path {
        ...
        for msg_ident in &bound_messages {
            ...
            if !available_methods.contains(&expected_method_name.as_str()) {
                let suggestion = crate::suggest_closest(
        ...
        auto_bind_logic = quote! {
            ...
            impl #impl_extended guinea::addr::UiAutoWire<B_PORT> for #self_ty #where_clause {
                const IS_COMPLETE: bool = #is_complete;

                #(#summary_attrs)*
                fn wire_full(addr: &guinea_core::actor::Addr<Self>, port: &B_PORT) {
                    #wire_full_call
                }

                #(#summary_attrs)*
                fn wire_partial(addr: &guinea_core::actor::Addr<Self>, port: &B_PORT) {
