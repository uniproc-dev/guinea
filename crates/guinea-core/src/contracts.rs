...
//! `#[port]`/`#[bindings]` macros (see `guinea-codegen::contracts`) at their
...
pub struct PortStubMeta {
    ...
    /// parsing in `guinea-codegen::contracts::parse_port_extra_methods`, and
    /// the stub-side handler+call-count codegen in
    /// `guinea-codegen::stub_gen::generate_port_extra_method`) exists only to
